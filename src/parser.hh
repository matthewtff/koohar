...
public:
	...
	template<class Predicate> void objects(const Predicate& Pred) { m_xml.objects(Pred); }
...
private:
	...
	bool readProperty(std::string& name, std::string& value, bool& object_closed);
	bool getCh(char& Ch);
	bool preCh(char& Ch);
	...
	struct ObjPredicate {
		ObjPredicate(const std::string& NewKooharTag, const std::string& NewPropName);
