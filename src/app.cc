...
void App::generateCookieKey(std::string& PlaceTo)
{
	PlaceTo.erase();
...
ThreadReturnValue listening_thread(ThreadGetValue TInfo)
{
	...
	typename App::ThreadInfo& thread_info = *(
		reinterpret_cast<typename App::ThreadInfo*>(TInfo)
	);
...
void App::listen (UserFunc UserCallFunction)
{
