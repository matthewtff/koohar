...
void WebPage::render ()
{
	...
	for (iterator it = begin(); it != end(); ++it) // Apply all changes to parsed dom
		m_dom.objects(ObjectPredicate<FindByRule, ApplyWebuiObject>(FindByRule(it->first), ApplyWebuiObject(it->second)));
...
void CastWebuiObject::operator() (webui::ObjectPtr Obj)
{
	...
	for (webui::StringMap::iterator it = Obj->begin(); it != Obj->end(); ++it) // Add all properties
...
void ApplyWebuiObject::operator()(xml::Object& Obj) const
{
	if (!m_obj->tag().empty()) // user did not specified new tag
		Obj.tag(m_obj->tag());
	for (StringMap::iterator it = m_obj->begin(); it != m_obj->end(); ++it)
	...
	m_search_by_id(false), m_search_by_class(false)
{
	if (m_rule[0] == '#') {
		m_search_by_id = true;
#ifdef _DEBUG
		std::cout << "Searching by id..." << std::endl;
#endif /* _DEBUG */
	} else if (m_rule[0] == '.') {
		m_search_by_class = true;
#ifdef _DEBUG
		std::cout << "Searching by class..." << std::endl;
#endif /* _DEBUG */
	}
